	...
	public class SDMFileManager
	{
		...
		public SDMFileManager ()
		{
			...
			if (!File.Exists (userFile)) {
				...
				File.WriteAllText (userFile, "0, 0, 0, 0");
			}
		...
		{
			String dataToWrite = updatedUser.TotalDistance + "," + updatedUser.TotalPoints + "," + updatedUser.TotalHardStops + "," + updatedUser.TotalHardStarts;
		...
		public User readUserFile ()
		{
			...
			User userData = new User (Double.Parse(splitDataFromFile[0]), int.Parse(splitDataFromFile[1]), int.Parse(splitDataFromFile[2]) , int.Parse(splitDataFromFile[3]));
			return userData;
		}
